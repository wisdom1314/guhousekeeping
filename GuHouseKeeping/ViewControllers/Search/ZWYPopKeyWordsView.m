...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [self.changeBtn setBackgroundImage:[UIImage imageNamed:@"coupons_qrscan.png"] forState:UIControlStateNormal];
        self.changeBtn.frame = CGRectMake(33, 341, 255, 40);
//        self.changeBtn.backgroundColor = [UIColor colorWithRed:241.0f/255.0f green:241.0f/255.0f blue:241.0f/255.0f alpha:1.0];
        [self.changeBtn setTitle:@"换一批" forState:UIControlStateNormal];
        [self.changeBtn.titleLabel setFont:[UIFont systemFontOfSize:15.0]];
        [self.changeBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
//        self.changeBtn.layer.borderColor = DARKGRAY.CGColor;
...
- (UIButton *)keyWordButtonWithTitle:(NSString *)aTitle tag:(NSInteger)aTag
{
    ...
    NSInteger color = (arc4random() % 246) + 10;
//    171 210
...
//    CGFloat green = (arc4random() % 246) + 10;
    ...
    [button setBackgroundColor:[UIColor colorWithRed:red/255.0f green:red/255.0f blue:red/255.0f alpha:1.0]];
...
{
    //
    if ([self.delegate respondsToSelector:@selector(searchHotTaglibWithKeyWord:)]) {
...
- (NSArray *)keyWordButtonAnimationFrameWithButtonArray:(NSArray *)aButtonArray
{
    ...
    for (int i = 0; i < [rowNoArray count]; i ++ ) {
        ...
        NSInteger button_y = (IPHONE5?20:0) + i * 70;
...
{
    NSInteger keyWordNo = maxKeyWord;//keyword个数
    ...
    for (int i = 0; i < row; i ++ ) {
        ...
        else
        {
            ...
            NSInteger rowNo;
            if (maxRowNo - minRowNo +1 != 0) {
                
                rowNo = (arc4random() % (maxRowNo - minRowNo +1)) + minRowNo;
...
- (NSArray *)rondomkeyWordArray
{
    if ([self.keyWordArray count] > maxKeyWord) {
        if (self.isFirst) {
            self.isFirst = NO;
            //第一次取前十个热词
            NSArray *keyWordArray = [self.keyWordArray objectsAtIndexes:[NSIndexSet indexSetWithIndexesInRange:NSMakeRange(0, maxKeyWord)]];
            return keyWordArray;
        }
        else
        {
